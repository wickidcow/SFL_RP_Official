...
def upstream_cases(data) -> dict[str, dict]:
    """Return every top-level string-CMD case generated by the Pylon pack.

    The upstream generator emits more than just pylon:* keys. Rebar, RebarMobs
    and block-display helpers use keys such as rebar:*, rebarmobs:*,
    fluid_point_input:*, fluid_point_output:*, fluid_point_intersection:* and
    fluid_pipe_display:*. Because old_pylon/new_pylon are both generated from
    the official upstream repository, any top-level string case present there
    is part of the upstream surface we need to diff.
    """
    ...
    for case in root.get("cases", []):
        ...
        if not isinstance(when, str):
            continue
        if when in out and canonical(out[when]) != canonical(case):
            raise ValueError(f"duplicate upstream CustomModelData case: {when}")
        out[when] = case
    return out
...
def main() -> int:
    ...
    # Never replace a combined minecraft/items file. Merge only the top-level
    # string CustomModelData cases emitted by the official Pylon generator.
    # This includes Pylon, Rebar, RebarMobs and generated helper namespaces.
    old_item_names = {
    ...
    for name in sorted(old_item_names | new_item_names):
        ...
        old_cases = upstream_cases(old_data)
        new_cases = upstream_cases(new_data)
        ...
        # Preserve the combined pack's existing case order. New upstream cases
        # are appended; existing cases are replaced in place. This minimizes
        # unrelated changes to shared carrier definitions.
